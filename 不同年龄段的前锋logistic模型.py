...
plt.title("25岁以下的前锋运动员最重要的五个指标")
joblib.dump(logreg,'25以下的前锋logistic模型.pkl')
plt.show()
...
data= pd.read_csv(r'C:\Users\86139\Desktop\football4.csv')
X =data[data['position']==1]
x1 =X[(X['Age']<30) &(X['Rating']>70)&(X['Age']>25)]
x2 =X[(X['Age']<30) &(X['Rating']<70)&(X['Age']>25)]
x1['position1']=1
x2['position1']=0
data11=pd.concat([x1,x2],ignore_index=True)
X =data11.iloc[:,12:-2]
...
students =(a[index].T).tolist()[0]
ax.bar(langs,students)
plt.title("25-30岁的前锋运动员最重要的五个指标")
joblib.dump(logreg,'25到30岁的前锋logistic模型.pkl')
plt.show()
...
students =(a[index].T).tolist()[0]
ax.bar(langs,students)
plt.title("30岁以上前锋运动员最重要的五个指标")
joblib.dump(logreg,'30岁以上的前锋logistic模型.pkl')
plt.show()