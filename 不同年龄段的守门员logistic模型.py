...
plt.title("25岁以下的守门员员最重要的五个指标")
joblib.dump(logreg,'25岁以下的守门员logistic模型.pkl')
plt.show()
scaler =StandardScaler()
from sklearn.preprocessing import StandardScaler
data= pd.read_csv(r'C:\Users\86139\Desktop\football4.csv')
...
students =(a[index].T).tolist()[0]
ax.bar(langs,students)
joblib.dump(logreg,'25到30岁的守门员logistic模型.pkl')
plt.title("25到30岁的守门员最重要的五个指标")
plt.show()
scaler =StandardScaler()
from sklearn.preprocessing import StandardScaler
data= pd.read_csv(r'C:\Users\86139\Desktop\football4.csv')
...
students =(a[index].T).tolist()[0]
ax.bar(langs,students)
plt.title("30岁以上的守门员最重要的五个指标")
joblib.dump(logreg,'30岁以上的守门员logistic模型.pkl')
plt.show()